...
class PIDT:
    def __init__(self, alpha, beta2, gamma, params=None, lr=2e-3, L=None, dz=None, dt=None, beta2_factor=None, gamma_factor=None, num_steps=None, T=None, lamio=10.0, fft_flag=0, loss_type='l2', noise_db=40):
    ...
    def pde_residual_net(self, params, s_esti_xt, x, t):
        ...
        def field(x, t):
            u = interpax.interp2d(x, t, grid_x, grid_y, np.real(s_esti_xt), method='cubic2', derivative=0)
            v = interpax.interp2d(x, t, grid_x, grid_y, np.imag(s_esti_xt), method='cubic2', derivative=0)
            return u, v
    ...
    def loss_pde_res(self, params, batch, io_batch):
        _, params_dt= params
        y = batch
        inputs, _ = io_batch

        # select input used for PDE residual
        idx = onp.random.randint(0, inputs.shape[0], (1,))

        _, s_esti_xt = self.ssfm(params_dt, inputs[idx][0], self.dt, self.dz, self.num_steps, self.A2)

        res1_pred, res2_pred = vmap(self.pde_residual_net, (None, None, 0, 0))(params, s_esti_xt, y[:, 0], y[:, 1])

        if self.loss_type=='l2':
            loss_res1 = np.mean((res1_pred) ** 2)
            loss_res2 = np.mean((res2_pred) ** 2)
        elif self.loss_type=='l1':
            loss_res1 = np.mean(np.abs(res1_pred))
            loss_res2 = np.mean(np.abs(res2_pred))
        else:
    ...
    #     inputs, _ = io_batch
    #     _, params_dt= params
    #     K = min(8, inputs.shape[0])
    #     idx = onp.random.choice(inputs.shape[0], (K,), replace=False)
    #     loss = 0.0
    #     for k in idx:
    #         _, s_esti_xt = self.ssfm(params_dt, inputs[k], self.dt, self.dz, self.num_steps, self.A2)
    #         y_k = y
    #         res1, res2 = vmap(self.pde_residual_net, (None, None, 0, 0))(params, s_esti_xt, y_k[:,0], y_k[:,1])
    #         if self.loss_type=='l2':
    #             loss += np.mean(res1**2) + np.mean(res2**2)
    #         elif self.loss_type=='l1':
    #             loss += np.mean(np.abs(res1)) + np.mean(np.abs(res2))
    #     return loss / K
...
def generate_one_pde_res_training_data(key,pres, L,Tw):
    ...
    x_res = random.uniform(subkeys[0], (pres, 1), minval=0.01*L, maxval=0.99*L)
    t_res = random.uniform(subkeys[1], (pres, 1), minval=0.01*Tw, maxval=0.99*Tw)
